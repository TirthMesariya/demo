...
async function fetchUserData() {
  try {
    ...
    const url = `https://krinik.in/user_match_get/user_id/${id}/`;
...
if (datefilter !== '' && datefilter !== 'Start & End Date') {
  ...
  var filteredArray = rankList.filter(function (object) {
    ...
    if (tab_filter_text !== '') {
      matchesText = (object.pool_name && object.pool_name.toString().toLowerCase().includes(tab_filter_text)) ||
      (object.pool_type && object.pool_type.toString().toLowerCase().includes(tab_filter_text)) ||
      (object.match.select_league.league_name && object.match.select_league.league_name.toString().toLowerCase().includes(tab_filter_text)) 

    }

    if (statusFilter !== 'All Status') {
      const status = getStatus(object.winning_status);
    ...
    // }
    if (startDate && endDate) {
    const objectDate = moment(object.user_data.date_time  , 'YYYY-MM-DD HH:mm:ss').toDate();
  ...
  if (!isNaN(object.total_amount)) {
    const amount = parseFloat(object.total_amount);
...
function getStatus(status) {
  if (status.toLowerCase() === "winner") {
    return "Win";  // Just return the text without HTML
  } else if (status.toLowerCase() === "contestant") {
    return "Lose";  // Just return the text without HTML
  } else if (status === "" || status === null) {
    return "Live";  // Just return the text without HTML
  } else {
    return "Unknown";  // Just return the text without HTML
  }
}
...
function displayTableRows() {
  ...
  for (var i = tab_start; i < tab_end; i++) {
    ...
    var poolNameCell = $("<td colspan='3'></td>").text(showdata["pool_name"] || "");
    var poolTypeCell = $("<td colspan='3'> </td>").text(showdata["pool_type"] || "");
    var userNameCell = $("<td colspan='3'> </td>").text(showdata.match.select_league["league_name"] || "");
    var mobileCell = $("<td colspan='3'> </td>").text(showdata["invest_amount"] || "");
    ...
    // if (credit_debitCell === "credit" || credit_debitCell.text === "Credit") {
      amountCell.html(`<span >${showdata["total_amount"] || ""}</span>`);
    // } 
    var statusCell = $("<td colspan='2'></td>").text(showdata["winning_status"]);
        if (showdata["winning_status"] === "winner" || showdata["winning_status"] === "Winner") {
      statusCell.html(`<span  style="color:green">Win</span>`);
    } else if (showdata["winning_status"] === "contestant" || showdata["winning_status"]=== "Contestant") {
      statusCell.html(`<span  style="color:#fbde08">lose</span>`);
    }else if (showdata["winning_status"] === "" || showdata["winning_status"]=== "") {
