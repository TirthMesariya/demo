...
const isStatusTrue = adminInfo?.status === "true";
  ...
  async function fetchData() {
      try {
          // Fetch teams data
          const teamsResponse = await fetch("https://krinik.in/scratch_coupon_get/", {
              method: "GET",
              headers: {
          ...
          if (teamsResponse.ok ) {
            ...
            //   const leaguesData = await leaguesResponse.json();
              ...
              if (teamsData.status === "success") {
                  rankList = teamsData.data;
                  console.log(rankList) // Update rankList with fetched teams data
...
function displayIndexButtons() {
  ...
  function displayTableRows() {
      ...
      for (var i = tab_start; i < tab_end; i++) {
          ...
          var logoCell = $("<td></td>").html(
              object["image"] ?
                  '<img src="https://krinik.in' + object["image"] + '" alt="' + object["image"] + '" class="team-logo lazyload">' :
                  ''
          );
          var dateCell = $("<td></td>").text(convertDateFormat(object["team_date"] || ""));
            ...
            // )
            var couponCell = $("<td ></td>").text(object["coupon_point"] || "");
          var editCell = $("<td class='otp-exempt3'></td>").html(
              '<span class=" otp-exempt3" onclick="handleEdit(' + object["id"] + ')"><i class="far fa-edit otp-exempt3"></i></span>'
          );
          var deleteCell = $("<td class='otp-exempt3'></td>").html(
              '<span class=" otp-exempt3" onclick="handleDelete(' + object["id"] + ')"><i class="far fa-trash-alt otp-exempt3"></i></span>'
          );
  ...
  addNewBtn.addEventListener("click",()=>{
    ...
    // refreshpage("addscratch.html")
    // window.location.href = "./addscratch.html"
  }
  })
        ...
        Promise.resolve()
            .then(async() => {
                if (confirm('Are you sure you want to delete this team?')) {
                    const url = `https://krinik.in/scratch_coupon_get/${id}/`;
                    try {
                        ...
                        if (response.ok) {
                           showDynamicAlert("Coupon Deleted Successfully !!")
            ...
            .then(async() => {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        window.location.href = `editscratch.html?scratchId=${id}`;
                    } else {
