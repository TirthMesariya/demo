...
// Dummy function for fetching data - replace with actual implementation
...
function filterRankList() {
  ...
  var filteredArray = rankList.filter(function (object) {
    ...
    if (tab_filter_text !== '') {
      matchesText = (object.username && object.username.toLowerCase().includes(tab_filter_text)) ||
...
function displayTableRows() {
  ...
  for (var i = tab_start; i < tab_end; i++) {
    ...
    var fullNameCell = $("<td colspan='3'></td>").text(showdata["username"] || "");
    var shortNameCell = $("<td colspan='3'> </td>").text(showdata["mobile_no"] || "");
    var emailCell = $("<td colspan='3'> </td>").text(showdata["transactions_id"] || "");
    var walletCell = $("<td colspan='2'> </td>").text(showdata["mode"] || "");
...
const workbook = XLSX.utils.table_to_book(table, { sheet: 'Wallet Transaction Data' });
const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
const data = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
const url = URL.createObjectURL(data);
const a = document.createElement('a');
a.href = url;
a.download = 'wallet_transaction_data.xlsx';
a.click();
