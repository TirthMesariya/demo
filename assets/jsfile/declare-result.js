      ...
      if (data && data.status === "success") {
        ...
        if (matchCheck && matchCheck.length > 0) {
          ...
          if (user_match_data1.length > 0) {
            ...
            user_match_data = user_match_data1.filter((p) => p.user_data && p.user_data.status === "unblock");
          } else {
  ...
  const allocateMoneyToWinners = async (matchScores, totalMoney) => {
    ...
    try {
      ...
      await $.ajax({
        ...
        success: (response) => {
          console.log(response,"succesfullyUpdatedWalletBalance")
        },
        error: (error) => {
